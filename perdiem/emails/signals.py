...
def sync_to_mailchimp_handler(sender, instance, **kwargs):
    user_is_subscribed_news = EmailSubscription.objects.is_subscribed(
        user=instance.user,
        subscription_type=EmailSubscription.SUBSCRIPTION_NEWS
    )
    if instance.subscription == EmailSubscription.SUBSCRIPTION_NEWS and instance.subscribed != user_is_subscribed_news:
